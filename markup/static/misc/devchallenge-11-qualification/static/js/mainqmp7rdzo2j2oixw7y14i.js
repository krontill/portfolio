...
(function () {
    var arrow = document.querySelector('[data-scroll="arrow"]');

    function scrollArrow() {
        var height = arrow.getBoundingClientRect().top + arrow.offsetHeight;
        if (height > 20) {
...
(function () {
    document.addEventListener('DOMContentLoaded', function () {
        ...
        var current = slider.querySelector('.js-current');
...
(function () {
    ...
    var btnToggle = document.querySelector('[data-toggle=toggle]');
...
(function () {
    window.onscroll = function () {
        var mas = document.querySelectorAll('[data-scroll="true"]');
        for (var i = 0; i < mas.length; i++) {
            var scrolled = window.pageYOffset || document.documentElement.scrollTop;
            var yPos = -(scrolled / mas[i].dataset.speed); // вычисляем коэффициент
...
(function () {
    document.addEventListener('DOMContentLoaded', function () {
        var slider3 = document.querySelector('.js_slider3');
        var partnersSlider = lory(slider3, {
            infinite: 6,
            enableMouseEvents: true
        });
...
(function () {
    document.addEventListener('DOMContentLoaded', function () {
        var slider2 = document.querySelector('.js_slider2');
        var dot_count = slider2.querySelectorAll('.js_slide').length;
        var dot_container = slider2.querySelector('.js_dots');
        var dot_list_item = document.createElement('li');

        function handleDotEvent(e) {
            if (e.type === 'before.lory.init') {
                for (var i = 0, len = dot_count; i < len; i++) {
                    var clone = dot_list_item.cloneNode(false);
                    dot_container.appendChild(clone);
                }
                dot_container.childNodes[0].classList.add('active');
            }
            if (e.type === 'after.lory.init') {
                for (var _i = 0, _len = dot_count; _i < _len; _i++) {
                    dot_container.childNodes[_i].addEventListener('click', function (e) {
            ...
            if (e.type === 'on.lory.resize') {
                for (var _i3 = 0, _len3 = dot_container.childNodes.length; _i3 < _len3; _i3++) {
                    dot_container.childNodes[_i3].classList.remove('active');
                }
                dot_container.childNodes[0].classList.add('active');
            }
        }
        ...
        var testimonialsSlider = lory(slider2, {
            infinite: 1,
            enableMouseEvents: true
        });
